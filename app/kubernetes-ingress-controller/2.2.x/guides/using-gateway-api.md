...
[Gateway API](https://gateway-api.sigs.k8s.io/) is a new set of resources for
...
Gateway API and Kong's implementation of Gateway API are both alpha stage and
...
values.yaml (set `CONTROLLER_FEATURE_GATES=Gateway=true` if not using Helm).
...
After creating an HTTPRoute, accessing `/echo` will forward a request to the
echo service:
